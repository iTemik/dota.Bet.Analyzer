...
def infer_primary_keys(tables: Dict[str, Iterable[Dict[str, str]]]) -> set:
    pk_candidates = set()
...
def generate_sql_from_json_text(json_text: str) -> str:
    ...
    index_columns = {'account_id', 'match_id', 'match_seq_num', 'leagueid', 'team_id', 'player_slot', 'api_key', 'hook_id'}

    sql_lines = []
    sql_lines.append('-- Auto-generated SQLite schema from backend/schema.txt')
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--check', action='store_true', help='Compare generated output to existing file and exit non-zero on mismatch')
